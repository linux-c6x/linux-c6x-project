                  ...
                  TMS320C6000 C/C++ CODE GENERATION TOOLS
                    0.3 ELF Dynamic Loader Release Notes
		               July 2009
...
1. The ELF Dynamic Loader
  ...
  1.2 Building the Dynamic Loader Tester (dl6x.6x)
...
loader.  This implementation is intended to be loaded and run on a C6x
...
1.2 Building the Dynamic Loader Tester (dl6x.<host>)
-------------------------------------------------------

Included in the alpha package is a Makefile which can be used to build 
the Dynamic Loader Tester (dl6x.6x).  As mentioned above, this
reference implementation of the ELF dynamic loader is intended to be
loaded and run on the C6x as a base image executable file.  The 
extension of the loader is the host platform on which it is to run.

With the latest alpha C6x v7.0 Alpha tools in your path and your C_DIR
and C6X_C_DIR environment variables set up to provide access to the RTS
header files and libraries, you can then build dl6x using 
gmake:

	%> gmake dl6x.6x

This will build an executable, dl6x.6x, which you can load and
start running with a version of CCS that supports loading a C6x ELF
...
We start by loading and starting the run of dl6x.6x as follows:
...
An example:
	...
	invoke dynamic loader:
		...
		%> kelvin_elf dl6x.6x --script dl_cmds.txt
...
When dl6x.6x is set running on the C6x, it enters a user-interactive
...
are built as the base image (dl6x.6x). This base image is first loader on the 
...
its dynamic symbols. In the system described here, this means the dl6x.6x is
reading itself.
...
Consider the build of the dl6x.6x file described above as an
